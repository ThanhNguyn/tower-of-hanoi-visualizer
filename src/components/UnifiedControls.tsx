...
export function UnifiedControls({
  ...
  return (
    <section aria-label="Game and Solver Controls" className="rounded-2xl border border-white/[0.08] bg-[#0d0f15] p-4 shadow-xl">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="hidden sm:inline text-xs text-slate-400 pl-2">
            {isSolved
              ? "🎉 Puzzle completed! Reset to play again."
              : selectedRod
              ? `Peg ${selectedRod} selected. Choose destination peg.`
              : "Click a peg to move, or use auto-solve."}
          </span>
        </div>

        {/* Right: Algorithm & Playback Transport */}
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1 rounded-lg border border-white/[0.08] bg-white/[0.02] p-1">
            <Cpu size={14} className="text-amber-400 ml-1.5 mr-0.5 hidden sm:block" />
            {ALGORITHMS.map((algo) => (
              <button
                key={algo.id}
                type="button"
